...
//uso array vuoti per suddividire le cadegorie in base alla lunghezza
$lunghe = [];
$corte = [];
$cortissime = [];
...
foreach ($cards as $card) {
    if ($card['tipo']== 'lunga') {
        $lunghe[] = $card; //sto pushando il tipo di pasta che è lunga nell'array $lunghe
...
@section('main')
            <div class="cards">
                    ...
                    @foreach ($lunghe as $card)
                        <div class="card">
                            <h3>{{$card['titolo']}}</h3> <!--uso [] perchè array mentre -> è per ogg-->
